...
// PRODUCT_CATEGORIES — 5 categorías REALES del live SEIND. Slugs sincronizados con
// TAXONOMY.categories de src/config/site.ts y con el `category` de cada producto .md.
...
// SERVICE_CATEGORIES — 4 servicios especializados REALES (asesoría, capacitación,
// mantenimiento, instalación). Slugs = id de SERVICES en site.ts y `category` del .md.
...
// ── Colección: casos — OMITIDA a propósito ───────────────────────────────────
// El live SEIND solo traía testimonios FABRICADOS (clientes/empresas inventados:
// "Roberto Martínez / Constructora Omega", etc.). Política B4 (cero fabricado): NO
// se define ni se exporta la colección `casos`. Si más adelante hay testimonios
// REALES y verificables, se reintroduce el schema y se exporta.
...
// `casos` (testimonios/casos de éxito) queda FUERA del export: el live SEIND solo
// traía testimonios FABRICADOS (clientes/empresas inventados) → no se portan (B4).
